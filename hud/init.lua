...
local function load()
    ...
    for address, data in pairs(glassData) do
        ...
        if data.notifications then table.insert(notificationsUsers, {component.proxy(address), {data.xRes or 2560, data.yRes or 1440}, data.scale or 3, data.offset or 0, data.backgroundColor or colors.darkGray, data.primaryColor or colors.electricBlue, data.accentColor or colors.accentColor, data.disabledColor or colors.yellow, data.maintColor or colors.red, data.displayDisabled}) end
    end
...
local function save()
    ...
    for address, data in pairs(glassData) do
        ...
        if data.notifications then
            table.insert(notificationsUsers, {component.proxy(address), {data.xRes or 2560, data.yRes or 1440}, data.scale, data.offset, data.backgroundColor, data.primaryColor, data.accentColor, data.disabledColor, data.maintColor, data.displayDisabled, data.notifYOffset})
